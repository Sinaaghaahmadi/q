  ...
  return (
    <div className="space-y-4">
              ...
              <table className="w-full min-w-[42rem] text-sm">
                <thead className="border-b border-ink-300/40 text-xs text-ink-600">
                  <tr>
                    ...
                    <th className="py-2 pe-4 text-start font-medium">{t("sanctions.col.list")}</th>
                    <th className="py-2 pe-4 text-start font-medium">{t("sanctions.col.score")}</th>
                    <th className="py-2 pe-4 text-start font-medium">
                      {t("sanctions.col.created")}
                    </th>
                    <th className="py-2 text-start font-medium">{t("sanctions.col.resolution")}</th>
                  </tr>
                </thead>
                <tbody>
                  {hits.map((hit) => (
                    <tr key={hit.id} className="border-b border-ink-300/25 align-top last:border-0">
                      <td className="py-3 pe-4">
                        {/* A possible sanctions match is the one row where
                            "who is this person" is the entire question, and
                            their file answers it. */}
                        <Link
                          href={`/admin/users/${hit.user_id}`}
                          className="font-medium hover:text-brand-700 dark:hover:text-brand-600"
                        >
                ...
                return (
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5">
                      <MessageSquareWarning className="size-4 shrink-0 text-warn-ink" aria-hidden />
      ...
      <PanelSection
        ...
        bodyClassName="space-y-2 text-sm leading-relaxed text-ink-600"
      >
