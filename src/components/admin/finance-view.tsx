...
export function FinanceView({
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("escrow.title")}</CardTitle>
        </CardHeader>
...
function BalanceBanner({
  ...
  return (
    <Card className={verdict && !balanced ? "border-down/50" : ""}>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          {!verdict ? (
            <Scale className="size-5 text-ink-600" aria-hidden />
          ) : balanced ? (
            <CircleCheck className="size-5 text-up" aria-hidden />
          ) : (
            <CircleAlert className="size-5 text-down" aria-hidden />
          )}
          {verdict
            ? balanced
              ? t("balance.zeroTitle")
...
function FeeRevenue({ months }: { months: FeeMonth[] }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("fees.platformTitle")}</CardTitle>
      </CardHeader>
...
function OfficeFees({ fees }: { fees: OfficeFee[] }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("fees.officeTitle")}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {fees.length === 0 ? (
...
function RecentTxns({ txns }: { txns: LedgerTxn[] }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("txn.title")}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {txns.length === 0 ? (
