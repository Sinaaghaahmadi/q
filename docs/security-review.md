...
- **21 functions are reachable from the API.** Five are read-only predicates the
  policies themselves evaluate; the rest are signed-in entry points that
  re-check the caller. Everything else — including every ledger posting and
  `assert_transition` — is closed to both `anon` and `authenticated`.
