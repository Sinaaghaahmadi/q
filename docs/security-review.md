...
| `staff_mfa_coverage()` shipped `SECURITY DEFINER` and granted to `authenticated` with no check on the caller, so any signed-in customer could read every staff member's name, roles, and **which of them had no second factor** — a list of whom to phish | fixed in 0030; gated on the two administrative seats                                    |
